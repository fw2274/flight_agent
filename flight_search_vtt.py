"""
Dual-agent flight search with Voice-to-Text integration.

Flow:
1) Voice input agent captures spoken flight requirements via MCP
2) Interpreter agent (LLM-only) turns natural language into clean parameters
3) Executor agent calls the LangGraph `search_flights` tool with those params

To run:
  # Voice input mode
  python flight_search_vtt.py --voice

  # Text input mode
  python flight_search_vtt.py --query "Find flights from LAX to NYC"

  # Voice with custom timeout
  python flight_search_vtt.py --voice --voice-timeout 45000
"""
...
# Import voice-to-text MCP client
try:
    from voice_mcp_client import VoiceToTextMCPClient
    VOICE_AVAILABLE = True
except ImportError:
    VOICE_AVAILABLE = False
    print("⚠️  voice_mcp_client not available. Voice input disabled.")
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Dual-agent Flight Search with Voice-to-Text (MCP)",
...
Examples:
  ...
  # Voice with longer timeout
  python flight_search_vtt.py --voice --voice-timeout 45000

  # Text input
  python flight_search_vtt.py --query "Find flights from LAX to NYC on Dec 15"

  # With debug output
  python flight_search_vtt.py --voice --debug --verbose
        """,
    )
    ...
    parser.add_argument("--query", type=str, help="Flight search query (text mode)")

    # Voice input options
    parser.add_argument("--voice", action="store_true", help="Use voice input via MCP")
    parser.add_argument(
        "--voice-timeout",
        type=int,
        default=30000,
        help="Voice recording timeout in ms (default: 30000)"
    )
    parser.add_argument(
        "--voice-silence-timeout",
        type=int,
        default=2000,
        help="Voice silence timeout in ms (default: 2000)"
    )
    parser.add_argument(
        "--mcp-server",
        type=str,
        default="voice-to-text-mcp/target/release/voice-to-text-mcp",
        help="Path to voice-to-text MCP server binary"
    )
    parser.add_argument(
        "--mcp-model",
        type=str,
...
def get_voice_input(args) -> str:
    """
    Capture voice input using MCP server.
    ...
    if not VOICE_AVAILABLE:
        print("❌ Voice input not available. Install voice_mcp_client.py")
        sys.exit(1)
    ...
    try:
        ...
        print(f"Model: {args.mcp_model}")
        print()

        client = VoiceToTextMCPClient(args.mcp_server, args.mcp_model)

        # Record and transcribe
        text = client.listen(
            timeout_ms=args.voice_timeout,
            silence_timeout_ms=args.voice_silence_timeout
        )

        if not text:
            print("❌ No voice input detected. Exiting.")
            sys.exit(1)
        ...
        print("2. Build MCP server:")
        print("   cd voice-to-text-mcp")
        print("   cargo build --release")
        print("3. Download Whisper model:")
...
async def main(args):
    ...
    # Get query: Auto-detect voice if no --query provided
    if args.query:
        # Text input explicitly provided
        query = args.query
        print("\n📝 Using text input")
    elif args.voice or VOICE_AVAILABLE:
        # Voice input (explicit --voice flag or automatic if available)
        query = get_voice_input(args)
        print("\n" + "=" * 60)
        ...
        # Fallback: No voice available and no query provided
        print("⚠️  Voice input not available (MCP server not found)")
        print("   To enable: See VOICE_SETUP.md")
        print("\n📝 Using default query for testing:")
        query = "Find a round-trip flight from ATL to JFK on 2025-12-01 returning 2025-12-15 for 2 adults in economy"
        print(f"   {query}")
